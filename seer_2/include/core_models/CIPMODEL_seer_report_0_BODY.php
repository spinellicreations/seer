...
---------------------------------------------------------------------
CIP REPORT 0 BODY (INCLUDED TO ALL CIPMODELS)
---------------------------------------------------------------------
*/
...
$apache_PAGETITLE = "
							<P CLASS='PAGETITLE'>
								<B>".$multilang_CIPMODEL_0.": ".$multilang_CIPMODEL_67."</B><BR>
...
if ( $seer_HMIACTION == 'BUILDTICKET' ) {
	...
	if ( $seer_HMIACTION_FAULT == 0 ) {
		...
		/* WASH QUERY */
		$mysql_mod_openopc_query = "DATESTAMP, CIPNAME, LINE_BEING_CLEANED, WATER_TYPE, WATER_USAGE";
		$mysql_mod_openopc_query = "SELECT ".$mysql_mod_openopc_query." FROM ".$CIPMODEL_mysql_mod_openopc_TABLENAME." WHERE (DATESTAMP BETWEEN '".$mysql_query_START_DATESTAMP."' AND '".$mysql_query_END_DATESTAMP."') AND (CIPNAME LIKE '".$CIPMODEL_PRESET_PREFIX."%') AND (STEP ".$CIPMODEL_STEP_REALSTEPS.") ORDER BY CIPNAME ASC, DATESTAMP ASC";
		...
		while ( $mysql_mod_openopc_query_row = mysqli_fetch_assoc($mysql_mod_openopc_query_result) ) {
			...
			$mysql_mod_openopc_WORKING_WATER_USAGE = $mysql_mod_openopc_query_row['WATER_USAGE'];
			/* -- CHECK IF THIS IS AN EXISTING RECORD (INSTANCE) OR A NEW ONE */
			if ( ($water[$discrete_water_instance_count]["CIPNAME"] == $mysql_mod_openopc_WORKING_CIPNAME) && ( $water[$discrete_water_instance_count]["LINEBEINGCLEANED"] >= 0 ) && ($water[$discrete_water_instance_count]["LINEBEINGCLEANED"] == $mysql_mod_openopc_WORKING_LINE_BEING_CLEANED) && ($water[$discrete_water_instance_count]["WATERTYPE"] == $mysql_mod_openopc_WORKING_WATER_TYPE) ) {
		...
		while ( $mysql_query_index <= $discrete_water_instance_count ) {
			...
			if ( $water[$mysql_query_index]["WATEREND"] > $water[$mysql_query_index]["WATERSTART"] ) {
				...
				$LINEBEINGCLEANED_EXAMINED_FRIENDLY = $CIPMODEL_LINE_BEING_CLEANED[$LINEBEINGCLEANED_EXAMINED];
				$WATERTYPE_EXAMINED_FRIENDLY = $CIPMODEL_WATER_TYPE[$WATERTYPE_EXAMINED];
				...
				if ( ($apache_REPORT_LAST_SYSTEM != $CIPNAME_EXAMINED) || ($apache_REPORT_LAST_LINE != $LINEBEINGCLEANED_EXAMINED_FRIENDLY) ) {
		...
		while ($mysql_query_index <= $CIPMODEL_COUNT_ADJUSTED ) {
			...
			while ($mysql_query_internal_index <= $CIPMODEL_WATER_TYPE_COUNT_ADJUSTED) {
				$TOTALWATER_TYPE_EXAMINED = $CIPMODEL_WATER_TYPE[$mysql_query_internal_index];
		...
		/* -- BUILD THE TOP PLATE */
		$apache_REPORT_RECORDENTRY_TOPPLATE = core_report_ticket_top_plate_and_export_link("","csv",$mysql_query_START_DATESTAMP,$mysql_query_END_DATESTAMP,$multilang_CIPMODEL_77);
								...
								<TR>
									<TD COLSPAN='7'>
										<P CLASS='INFOREPORTBIGTEXT'>
											[".$multilang_STATIC_AGGREGATE."] ".$multilang_CIPMODEL_78."...
										</P>
									</TD>
								</TR>
								<TR>
									...
									<TD ALIGN='LEFT'>
										<B><U>".$multilang_CIPMODEL_71."</U></B>
									</TD>
									<TD ALIGN='LEFT'>
										<B><U>".$multilang_CIPMODEL_37."</U></B>
									</TD>
									<TD ALIGN='LEFT'>
										<B><U>".$multilang_CIPMODEL_64." [".$CIPMODEL_UM_WATER."]</U></B>
									</TD>
								</TR>
								";
		/* -- ADD TOTALS THEN THE REST OF THE REPORT BODY */
		$apache_REPORT_RECORDENTRY = $apache_REPORT_RECORDENTRY_BODY_TOTALS."
								<TR>
									<TD COLSPAN='7'>
										<BR>
										<P CLASS='INFOREPORTBIGTEXT'>
										[".$multilang_STATIC_DISCRETE."] ".$multilang_CIPMODEL_79."...
										</P>
									</TD>
								</TR>
								<TR>

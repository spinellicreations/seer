...
---------------------------------------------------------------------
CIP REPORT 3 BODY (INCLUDED TO ALL CIPMODELS)
---------------------------------------------------------------------
*/
...
$apache_PAGETITLE = "
							<P CLASS='PAGETITLE'>
								<B>".$multilang_CIPMODEL_0.": ".$multilang_CIPMODEL_7."</B><BR>
...
if ( $seer_HMIACTION == 'BUILDTICKET' ) {
	...
	if ( $mysql_ENTRY_DISPLAY_UNDER_DECLARED_ALARM_CONDITION_ONLY == 'DEFAULT_PROCESSALLALARMS' ) {
		...
		$mysql_ENTRY_DISPLAY_UNDER_DECLARED_ALARM_CONDITION_ONLY_SELECT = "STATUS ".$CIPMODEL_STATUS_FAULT;
		$REPORT_TICKET_ALARM_ID = $multilang_STATIC_94;
	} else {
		/* DISPLAY UNDER ONLY THE USER SELECTED ALARM CONDITION */
		$mysql_ENTRY_DISPLAY_UNDER_DECLARED_ALARM_CONDITION_ONLY_SELECT = "STATUS = ".$mysql_ENTRY_DISPLAY_UNDER_DECLARED_ALARM_CONDITION_ONLY;
		$REPORT_TICKET_ALARM_ID = $CIPMODEL_STATUS[$mysql_ENTRY_DISPLAY_UNDER_DECLARED_ALARM_CONDITION_ONLY];
	}
	...
	if ( $seer_HMIACTION_FAULT == 0 ) {
		...
		list($fault_container_TOPRATED,$machine_fault_index_container_TOPRATED) = model_COMMON_top_rated_fault_container_creation_report_3($CIPMODEL_COUNT_ADJUSTED,$machine_fault_index_container,$fault_container,$endtime_container,$starttime_container,$duration_container_human_readable,$duration_container_unixtime);
		...
		if ( $machine_fault_index_container_TOPRATED == 0 ) {
			...
			if ( $mysql_ENTRY_DISPLAY_UNDER_DECLARED_ALARM_CONDITION_ONLY == 'DEFAULT_PROCESSALLALARMS' ) {
				...
				$apache_PARETO = core_pareto_determination_and_plot("DURATION",$multilang_STATIC_FAULTS_CAPS, $CIPMODEL_STATUS_COUNT_ADJUSTED, $CIPMODEL_STATUS, $fault_container_TOTAL_TOPRATED);
			...
			if ( $mysql_ENTRY_DISPLAY_UNDER_DECLARED_ALARM_CONDITION_ONLY == 'DEFAULT_PROCESSALLALARMS' ) {
				...
				$apache_PARETO = core_pareto_determination_and_plot("FREQUENCY",$multilang_STATIC_FAULTS_CAPS, $CIPMODEL_STATUS_COUNT_ADJUSTED, $CIPMODEL_STATUS, $fault_container_TOTAL_TOPRATED);
				$apache_REPORT_RECORDENTRY = $apache_REPORT_RECORDENTRY.$apache_PARETO;
			} else {
				/* pass */
			}
			...
			while ( ($mysql_query_internal_index <= $machine_fault_index_container_TOPRATED) && ($mysql_query_internal_index <= 24) ) {
				...
				if ( $fault_container_TOPRATED_CHECK == TRUE ) {
					...
					$MACHINEMODEL_WORKING_POST_MACHINE = $CIPMODEL_NAME[$MACHINEMODEL_WORKING_POST_MACHINE];
					$MACHINEMODEL_WORKING_POST_FAULT = $CIPMODEL_STATUS[$MACHINEMODEL_WORKING_POST_FAULT];
			...
			while ( $mysql_query_internal_index <= $CIPMODEL_COUNT_ADJUSTED ) {
				...
				while ( $machine_fault_index_container_CYCLE <= $machine_fault_index_container[$MODEL_WORKING_MACHINENAME] ) {
					...
					if ( ($fault_container_CHECK == TRUE) && ($fault_container_CHECK_2 != 0) ) {
						...
						$MACHINEMODEL_WORKING_POST_MACHINE = $CIPMODEL_NAME[$MODEL_WORKING_MACHINENAME];
		...
		if ( ($apache_ZERO_FAULT_OUTPUT == 'NO') && ($mysql_ENTRY_DISPLAY_UNDER_DECLARED_ALARM_CONDITION_ONLY == 'DEFAULT_PROCESSALLALARMS') ) {
			$apache_REPORT_RECORDENTRY_TOPPLATE_EXTENSION = $apache_REPORT_RECORDENTRY_TOPPLATE_EXTENSION."
								<TR>
									<TD ALIGN='CENTER'>
		...
		/* -- -- OPTION FULFILLMENT */
		$custom_array_of_option_names="<OPTION VALUE='DEFAULT_PROCESSALLALARMS'>".$multilang_STATIC_94.$CIPMODEL_FORMFILL_STATUS;
		/* -- -- AND */
		/* -- CLOSE OUT REPORT */
		$apache_REPORT_RECORDENTRY = $apache_REPORT_RECORDENTRY."

...
---------------------------------------------------------------------
CIP REPORT 4 BODY (INCLUDED TO ALL CIPMODELS)
---------------------------------------------------------------------
*/
...
$apache_PAGETITLE = "
							<P CLASS='PAGETITLE'>
								<B>".$multilang_CIPMODEL_0.": ".$multilang_CIPMODEL_8."</B><BR>
...
if ( $seer_HMIACTION == 'BUILDTICKET' ) {
	...
	if ( $seer_HMIACTION_FAULT == 0 ) {
		...
		/* PULL IN DATA FOR CIP STATE */
		/* -- QUERY */
		$mysql_mod_openopc_query = "DATESTAMP, LINE_BEING_CLEANED";
		$mysql_mod_openopc_query = "SELECT ".$mysql_mod_openopc_query." FROM ".$CIPMODEL_mysql_mod_openopc_TABLENAME." WHERE (DATESTAMP BETWEEN '".$mysql_query_START_DATESTAMP."' AND '".$mysql_query_END_DATESTAMP."') AND (CIPNAME = '".$mysql_ENTRY_MACHINENAME."') AND (LINE_BEING_CLEANED" .$CIPMODEL_LINE_BEING_CLEANED_REALLINES .") AND (STEP ".$CIPMODEL_STEP_REALSTEPS.") ORDER BY DATESTAMP ASC";
		...
		$apache_ZERO_FAULT_DETECTED = model_COMMON_event_container_fill_report_4($apache_ZERO_FAULT_DETECTED,$mysql_ENTRY_MACHINENAME,$mysql_mod_openopc_query_result,"LINE_BEING_CLEANED",$CIPMODEL_LINE_BEING_CLEANED,"1");
		/* PULL IN DATA FOR CIP STATUS and FAULTS */
		$mysql_mod_openopc_query = "DATESTAMP, STATUS";
		$mysql_mod_openopc_query = "SELECT ".$mysql_mod_openopc_query." FROM ".$CIPMODEL_mysql_mod_openopc_TABLENAME." WHERE (DATESTAMP BETWEEN '".$mysql_query_START_DATESTAMP."' AND '".$mysql_query_END_DATESTAMP."') AND (CIPNAME = '".$mysql_ENTRY_MACHINENAME."') AND (STATUS ".$CIPMODEL_STATUS_FAULT.") AND (STEP ".$CIPMODEL_STEP_REALSTEPS.") ORDER BY DATESTAMP ASC";
		list($mysql_mod_openopc_query_result,$mysql_mod_openopc_num_rows) = core_mysql_mod_openopc_query_shell($mysql_mod_openopc_query);
		/* -- ADD TO EVENT CONTAINER */
		$apache_ZERO_FAULT_DETECTED = model_COMMON_event_container_fill_report_4($apache_ZERO_FAULT_DETECTED,$mysql_ENTRY_MACHINENAME,$mysql_mod_openopc_query_result,"STATUS",$CIPMODEL_STATUS,"2");
...
if ( $seer_HMIACTION == "DISPLAY_START_PAGE" ) {
	$apache_REPORT_RECORDENTRY = core_user_date_time_range_prompt_type_1("BUILDTICKET",$multilang_CIPMODEL_29,$CIPMODEL_FORMFILL_NAME,"NULL",$multilang_STATIC_REPORT_TIME);
}

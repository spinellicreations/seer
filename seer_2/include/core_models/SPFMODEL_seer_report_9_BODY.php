...
---------------------------------------------------------------------
SPF REPORT 9 BODY (INCLUDED TO ALL SPFMODELS)
---------------------------------------------------------------------
*/
...
$apache_PAGETITLE = "
							<P CLASS='PAGETITLE'>
								<B>".$multilang_SPFMODEL_0.": ".$multilang_SPFMODEL_14."</B><BR>
...
if ( $seer_HMIACTION == 'BUILDTICKET' ) {
	...
	if ( $seer_HMIACTION_FAULT == 0 ) {
		...
		/* PULL IN DATA FOR STATE */
		/* -- QUERY */
		$mysql_mod_openopc_query = "DATESTAMP, STATE";
		$mysql_mod_openopc_query = "SELECT ".$mysql_mod_openopc_query." FROM ".$SPFMODEL_mysql_mod_openopc_TABLENAME." WHERE (DATESTAMP BETWEEN '".$mysql_query_START_DATESTAMP."' AND '".$mysql_query_END_DATESTAMP."') AND (MACHINE_NAME = '".$mysql_ENTRY_MACHINENAME."') ORDER BY DATESTAMP ASC";
		...
		$apache_ZERO_FAULT_DETECTED = model_COMMON_event_container_fill_report_4($apache_ZERO_FAULT_DETECTED,$mysql_ENTRY_MACHINENAME,$mysql_mod_openopc_query_result,"STATE",$SPFMODEL_STATE,"0");

		/* PULL IN DATA FOR FAULTS and ALARMS */
		/* -- QUERY */
		$mysql_mod_openopc_query = "DATESTAMP, ALARM";
		$mysql_mod_openopc_query = "SELECT ".$mysql_mod_openopc_query." FROM ".$SPFMODEL_mysql_mod_openopc_TABLENAME." WHERE (DATESTAMP BETWEEN '".$mysql_query_START_DATESTAMP."' AND '".$mysql_query_END_DATESTAMP."') AND (MACHINE_NAME = '".$mysql_ENTRY_MACHINENAME."') AND (ALARM > '0') ORDER BY DATESTAMP ASC";
		list($mysql_mod_openopc_query_result,$mysql_mod_openopc_num_rows) = core_mysql_mod_openopc_query_shell($mysql_mod_openopc_query);
		/* -- ADD TO EVENT CONTAINER */
		$apache_ZERO_FAULT_DETECTED = model_COMMON_event_container_fill_report_4($apache_ZERO_FAULT_DETECTED,$mysql_ENTRY_MACHINENAME,$mysql_mod_openopc_query_result,"ALARM",$SPFMODEL_ALARM,"2");
...
if ( $seer_HMIACTION == "DISPLAY_START_PAGE" ) {
	$apache_REPORT_RECORDENTRY = core_user_date_time_range_prompt_type_1("BUILDTICKET",$multilang_SPFMODEL_58,$SPFMODEL_FORMFILL_NAME,"NULL",$multilang_STATIC_REPORT_TIME);
}

...
---------------------------------------------------------------------
TANK REPORT 4 BODY (INCLUDED TO ALL TANKMODELS)
---------------------------------------------------------------------
*/
...
$apache_PAGETITLE = "
							<P CLASS='PAGETITLE'>
								<B>".$multilang_TANKMODEL_0.": ".$multilang_TANKMODEL_10."</B><BR>
...
if ( $seer_HMIACTION == 'BUILDTICKET' ) {
	...
	if ( $mysql_ENTRY_DISPLAY_UNDER_DECLARED_ALARM_CONDITION_ONLY == 'DEFAULT_PROCESSALLALARMS' ) {
		...
		$REPORT_TICKET_ALARM_ID = $TANKMODEL_ALARM[$mysql_ENTRY_DISPLAY_UNDER_DECLARED_ALARM_CONDITION_ONLY];
	}
	...
	if ( $seer_HMIACTION_FAULT == 0 ) {
		...
		list($fault_container_TOPRATED,$machine_fault_index_container_TOPRATED) = model_COMMON_top_rated_fault_container_creation_report_3($TANKMODEL_COUNT_ADJUSTED,$machine_fault_index_container,$fault_container,$endtime_container,$starttime_container,$duration_container_human_readable,$duration_container_unixtime);
		...
		if ( $machine_fault_index_container_TOPRATED == 0 ) {
			...
			if ( $mysql_ENTRY_DISPLAY_UNDER_DECLARED_ALARM_CONDITION_ONLY == 'DEFAULT_PROCESSALLALARMS' ) {
				...
				$apache_PARETO = core_pareto_determination_and_plot("DURATION",$multilang_STATIC_FAULTS_CAPS, $TANKMODEL_ALARM_COUNT_ADJUSTED, $TANKMODEL_ALARM, $fault_container_TOTAL_TOPRATED);
			...
			if ( $mysql_ENTRY_DISPLAY_UNDER_DECLARED_ALARM_CONDITION_ONLY == 'DEFAULT_PROCESSALLALARMS' ) {
				...
				$apache_PARETO = core_pareto_determination_and_plot("FREQUENCY",$multilang_STATIC_FAULTS_CAPS, $TANKMODEL_ALARM_COUNT_ADJUSTED, $TANKMODEL_ALARM, $fault_container_TOTAL_TOPRATED);
				$apache_REPORT_RECORDENTRY = $apache_REPORT_RECORDENTRY.$apache_PARETO;
			} else {
				/* pass */
			}
			...
			while ( ($mysql_query_internal_index <= $machine_fault_index_container_TOPRATED) && ($mysql_query_internal_index <= 24) ) {
				...
				if ( $fault_container_TOPRATED_CHECK == TRUE ) {
					...
					$MACHINEMODEL_WORKING_POST_MACHINE = $TANKMODEL_NAME[$MACHINEMODEL_WORKING_POST_MACHINE];
					$MACHINEMODEL_WORKING_POST_FAULT = $TANKMODEL_ALARM[$MACHINEMODEL_WORKING_POST_FAULT];
		...
		/* -- -- OPTION FULFILLMENT */
		$custom_array_of_option_names="<OPTION VALUE='DEFAULT_PROCESSALLALARMS'>".$multilang_STATIC_94.$TANKMODEL_FORMFILL_ALARM;
		/* -- -- AND */
		/* -- CLOSE OUT REPORT */
		$apache_REPORT_RECORDENTRY = $apache_REPORT_RECORDENTRY."
